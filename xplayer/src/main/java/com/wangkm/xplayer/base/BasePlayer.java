...
import com.wangkm.xplayer.media.IVideoPlayer;
import com.wangkm.xplayer.model.PlayerState;
import com.wangkm.xplayer.widget.view.ScreenOrientationRotate;
...
public abstract class BasePlayer extends FrameLayout implements IPlayerControl, IBasePlayer, ScreenOrientationRotate.OnDisplayOrientationListener {
    ...
    private void hideSystemBar(ViewGroup decorView) {
        Activity activity = PlayerUtils.INSTANCE.getActivity(getTargetContext());
    ...
    public void onPlayerState(PlayerState state, String message) {
        ILogger.INSTANCE.d(TAG,"onPlayerState-->state:"+state+",message:"+message);
...
//        ILogger.d(TAG,"progress-->currentDurtion:"+currentDurtion+",totalDurtion:"+totalDurtion);
    ...
    public void setController(BaseController controller) {
        PlayerUtils.INSTANCE.removeViewFromParent(mController);
        this.mController=controller;
        PlayerUtils.INSTANCE.removeViewFromParent(controller);
    ...
    public void setInterceptTAudioFocus(boolean interceptTAudioFocus) {
        if(null!=mIVideoPlayer){
            ...
            IVideoManager.INSTANCE.setInterceptTAudioFocus(interceptTAudioFocus);
        }
    }
...
//        ILogger.d(TAG,"startFullScreen");
        ...
        if (null != activity&& !activity.isFinishing()) {
            ...
            PlayerUtils.INSTANCE.removeViewFromParent(this);//从原宿主中移除自己
...
//        ILogger.d(TAG,"quitLandscapeScreen");
        ...
        if(null!=activity&&!activity.isFinishing()){
            ...
            //1:从Window窗口中移除自己
            PlayerUtils.INSTANCE.removeViewFromParent(this);
            //2.改变屏幕方向为竖屏
...
//                ILogger.d(TAG,"quitLandscapeScreen-->退出全屏无宿主接收,销毁播放器");
    ...
    public void startWindow(int width, int height, float startX, float startY, float radius, int bgColor, boolean isAutoSorption) {
        ILogger.INSTANCE.d(TAG,"startWindow-->width:"+width+",height:"+height+",startX:"+startX+",startY:"+startY+",radius:"+radius+",bgColor:"+bgColor+",windowProperty:"+ mIsActivityWindow +",screenOrientation:"+mScreenOrientation+",isAutoSorption:"+isAutoSorption);
...
//                ILogger.d(TAG,"startWindow-->parent_id:"+getId()+",parentX:"+screenLocation[0]+",parentY:"+screenLocation[1]+",parentWidth:"+mParent.getWidth()+",parentHeight:"+mParent.getHeight());
            ...
            if(width<=0){
                width = PlayerUtils.INSTANCE.getScreenWidth(getContext())/2+PlayerUtils.INSTANCE.dpToPxInt(30f);
                height = width*9/16;
//                ILogger.d(TAG,"startWindow-->未传入宽高,width:"+width+",height:"+height);
            ...
            if(startX<=0&&null!=mParent){
                startX=(PlayerUtils.INSTANCE.getScreenWidth(getContext())/2-PlayerUtils.INSTANCE.dpToPxInt(30f))-PlayerUtils.INSTANCE.dpToPxInt(12f);
                startY=screenLocation[1]+mParent.getHeight()+PlayerUtils.INSTANCE.dpToPxInt(12f);
//                ILogger.d(TAG,"startWindow-->未传入X,Y轴,取父容器位置,startX:"+startX+",startY:"+startY);
            ...
            if(startX<=0){
                startX=(PlayerUtils.INSTANCE.getScreenWidth(getContext())/2-PlayerUtils.INSTANCE.dpToPxInt(30f))-PlayerUtils.INSTANCE.dpToPxInt(12f);
                startY=PlayerUtils.INSTANCE.dpToPxInt(60f);
//                ILogger.d(TAG,"startWindow-->未传入X,Y轴或取父容器位置失败,startX:"+startX+",startY:"+startY);
            }
            ILogger.INSTANCE.d(TAG,"startWindow-->final:width:"+width+",height:"+height+",startX:"+startX+",startY:"+startY);
...
//            ILogger.d(TAG,"quitWindow-->getId():"+getId());
            PlayerUtils.INSTANCE.removeViewFromParent(playerView);//移除拖拽窗口View
            PlayerUtils.INSTANCE.removeViewFromParent(this);//从拖拽窗口View中移除播放器
            //2.改变窗口状态
...
//                ILogger.d(TAG,"quitWindow-->退出窗口无宿主接收,通知控制器返回");
    ...
    public boolean startGlobalWindow(int width, int height, float startX, float startY, float radius, int bgColor, boolean isAutoSorption) {
        ILogger.INSTANCE.d(TAG,"startGlobalWindow-->width:"+width+",height:"+height+",startX:"+startX+",startY:"+startY+",radius:"+radius+",bgColor:"+bgColor +",screenOrientation:"+mScreenOrientation);
        if(mScreenOrientation==IMediaPlayer.ORIENTATION_LANDSCAPE){
            return false;
        }
        boolean existPermission = PlayerUtils.INSTANCE.existPermission(getContext(), Manifest.permission.SYSTEM_ALERT_WINDOW);//检查开发者是否申明悬浮窗权限
        boolean hasPermission = PlayerUtils.INSTANCE.checkWindowsPermission(getContext());//检查是否获取了悬浮窗权限
...
//                ILogger.d(TAG,"startGlobalWindow-->上下文为空,开启失败");
            ...
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    intent.setData(Uri.parse( "package:"+ PlayerUtils.INSTANCE.getPackageName(
                            getContext().getApplicationContext())));
                } else {
                    intent.setAction("android.settings.APPLICATION_DETAILS_SETTINGS");
                    intent.setData(Uri.fromParts("package", PlayerUtils.INSTANCE.getPackageName(
