...
public final class OkHttpUtils {
    ...
    private class Requst{
        ...
        public void setdRequst(final Request request, final OnResultCallBack callBack, boolean isSynchro) {
            if(DEBUG){
                Logger.INSTANCE.d(TAG,"setdRequst-->URL:"+request.url()+",isSynchro:"+isSynchro);
            }
            ...
            createHttpUtils().newCall(request).enqueue(new Callback() {
                ...
                public void onFailure(okhttp3.Call call, IOException e) {
                    if(DEBUG)Logger.INSTANCE.d(TAG,"onFailure-->e:"+e.getMessage()+call.toString());
                    isRequst=false;
                    error(ERROR_IO,e.getMessage());
                }
                    ...
                    if(REQUST_OK==response.code()){
                        try {
                            ...
                            if(!TextUtils.isEmpty(string)){
                                if(DEBUG) Logger.INSTANCE.d(TAG,"服务端返回数据-->"+string);
    ...
    private void download(final String path, String outPutPath, String outPutFileName, final OnDownloadListener listener) {
        ...
        createHttpUtils().newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(okhttp3.Call call, final IOException e) {
                if(DEBUG){
                    Logger.INSTANCE.d(TAG,"onFailure-->e:"+e.getMessage()+call.toString());
                }
                isRequst=false;
                    ...
                    if(null!=response){
                        if(200==response.code()){
                            try {
                                ...
                                Logger.INSTANCE.d(TAG,"目标存储路径："+apkDownloadPath.getAbsolutePath());
